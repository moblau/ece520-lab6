  ...
  // enable the interrupts to the PS here
   ...
   while (1) {
      ...
      switch (readVal) {
         ...
         case 'p':               // "pause" (pauses current count without changing the value of the timer)
            XTtcPs_Stop(&TtcPsInst);
            PRINT("Paused at %d:%02d.%02d\r",minutes,seconds,hsecs);
            break;

         case 's':               // "stop" (stops the timer and resets)
            XTtcPs_Stop(&TtcPsInst);
         ...
         case 'r':               // "reset" (drives the timer counters to zero) - even when running
            minutes=0; seconds=0; hsecs = 0;
            break;
